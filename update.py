...
def get_all_patients():
    data = load_data()
    return {
        pid: Patient(id=pid, **p).model_dump()
...
def get_patient(pid: str):
    ...
    return Patient(id=pid, **data[pid]).model_dump()
...
def add_patient(patient: Patient):
    ...
    # save only raw fields
    data[patient.id] = patient.model_dump(exclude={"bmi", "verdict"})
    save_data(data)
    return patient.model_dump()  # return with computed fields

# PUT/PATCH update an existing patient
...
def update_patient(pid: str, patient_update: PatientUpdate):
    ...
    # Return updated patient with computed fields
    return Patient(id=pid, **data[pid]).model_dump()
